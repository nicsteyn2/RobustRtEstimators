...
# Load the data
df = rbind(read.csv("paper/outputs/s9epiestim_siuncertainty.csv"), read.csv("paper/outputs/s9epifilter_siuncertainty.csv"))
df_params = rbind(
  read.csv("paper/outputs/s9epifilter_siuncertainty_params.csv") %>% rename(x=eta, y=peta),
  read.csv("paper/outputs/s9epiestim_siuncertainty_params.csv") %>% rename(x=k, y=pk)
)
...
plt_with_legend
ggsave(paste0("paper/figures/sx_gentimeuncertainty.png"), plt_with_legend, dpi=300, width=20, height=14, units="cm")
