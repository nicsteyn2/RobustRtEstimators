...
# install.packages("EpiNow2", version="1.6.1")
...
df_truth = df %>% rename(R = TrueRt, I = Cases) %>% pivot_longer(cols=c("R", "I"), names_to="variable", values_to="TrueValue") %>% dplyr::select(-date)
df_results = left_join(df_results, df_truth, by=c("t", "simulation", "variable"))

# Save results
write.csv(df_results, "othermethods/EpiNow2/results.csv", row.names=FALSE)
write.csv(df_params, "othermethods/EpiNow2/params.csv", row.names=FALSE)
